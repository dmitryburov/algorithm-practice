...
Для решения задачи я используется поиск по циклам в графе: если есть цикл длины 3,
значит существует 2 способа поспать из вершины №1 в вершину №3 - через вершину №2 по одному типу дороги,
и напрямую по другому => в таком случае карта с такими путями не будет оптимальной.

Для хранения вершин используется список смежности.
...
Cложность будет ассимптотическая: O(n + m).
